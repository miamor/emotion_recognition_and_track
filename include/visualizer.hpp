...
public:
    ...
    explicit EmotionBarVisualizer(std::vector<std::string> const& emotionNames, std::vector<int> const& emotionCodes, 
                              cv::Size size = cv::Size(300, 140), cv::Size padding = cv::Size(10, 10),
                              double opacity = 0.6, double textScale = 1, int textThickness = 1);

    void draw(cv::Mat& img, std::map<int, float> emotions, cv::Point org, cv::Scalar fgcolor, cv::Scalar bgcolor);
    cv::Size getSize();
private:
...
public:
    ...
    void enableEmotionBar(std::vector<std::string> const& emotionNames, std::vector<int> const& emotionCodes);

...
EmotionBarVisualizer::EmotionBarVisualizer(std::vector<std::string> const& emotionNames, std::vector<int> const& emotionCodes, 
                                     cv::Size size, cv::Size padding,
...
void EmotionBarVisualizer::draw(cv::Mat& img, std::map<int, float> emotions, cv::Point org, cv::Scalar fgcolor, cv::Scalar bgcolor) {
    ...
    for (size_t i = 0; i< emotionCodes.size(); i++) {
        drawEmotion(i, emotionNames[i], emotions[emotionCodes[i]]);
    }
}
...
void Visualizer::enableEmotionBar(std::vector<std::string> const& emotionNames, std::vector<int> const& emotionCodes) {
    emotionVisualizer = std::make_shared<EmotionBarVisualizer>(emotionNames, emotionCodes);
...
void Visualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar) {
    ...
    if (f->isEmotionsEnabled()) {
        ...
        if (f->isAgeGenderEnabled()) out << ", ";
        out << EmotionsDetection::emotionsVecTxt[emotion.first];
    }

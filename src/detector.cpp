...
const std::vector<int> EmotionsDetection::emotionsVec = {0,1,2,3,4};
const std::vector<std::string> EmotionsDetection::emotionsVecTxt = {"Binh thuong", "Vui", "Buon", "Ngac nhien", "Tuc gian"};
...
void FaceDetection::enqueue(const cv::Mat &frame, const int frame_idx) {
    if (!enabled()) return;
...
void FaceDetection::fetchResults() {
    ...
    resultsFetched = true;
    const float *data = request->GetBlob(output)->buffer().as<float *>();

    for (int i = 0; i < maxProposalCount; i++) {
        const int start_pos = i * objectSize;
        ...
        const float x0 =
            std::min(std::max(0.0f, data[start_pos + 3]), 1.0f) * width;
        const float y0 =
            std::min(std::max(0.0f, data[start_pos + 4]), 1.0f) * height;
        const float x1 =
            std::min(std::max(0.0f, data[start_pos + 5]), 1.0f) * width;
        const float y1 =
            std::min(std::max(0.0f, data[start_pos + 6]), 1.0f) * height;
        ...
        if (object.confidence >= detectionThreshold && object.rect.area() > 0) {
        ...
        r.rect.x = static_cast<int>(detections[i * objectSize + 3] * width);
        r.rect.y = static_cast<int>(detections[i * objectSize + 4] * height);
        r.rect.width = static_cast<int>(detections[i * objectSize + 5] * width - r.rect.x);
        r.rect.height = static_cast<int>(detections[i * objectSize + 6] * height - r.rect.y);
        ...
        r.rect.x = bb_center_x - static_cast<int>(std::floor(bb_dx_coefficient * bb_new_width / 2));
        r.rect.y = bb_center_y - static_cast<int>(std::floor(bb_dy_coefficient * bb_new_height / 2));

        r.rect.width = bb_new_width;
        r.rect.height = bb_new_height;

        if (doRawOutputMessages) {
            std::cout << "[" << i << "," << r.label << "] element, prob = " << r.confidence <<
                         "    (" << r.rect.x << "," << r.rect.y << ")-(" << r.rect.width << ","
                      << r.rect.height << ")"
...
std::map<int, float> EmotionsDetection::operator[] (int idx) const {
    // Vector of supported emotions
    // static const std::vector<std::string> emotionsVec = {"0", "1", "2", "3", "4"};
    // static const std::vector<std::string> emotionsVecTxt = {"Binh thuong", "Vui", "Buon", "Ngac nhien", "Tuc gian"};
    ...
    for (size_t i = 0; i < emotionsVecSize; i++) {
        ...
        if (doRawOutputMessages) {
            std::cout << emotionsVecTxt[i] << " = " << outputIdxPos[i];
...
HeadPoseDetection::Results HeadPoseDetection::operator[](int idx) const
{
    ...
    if (doRawOutputMessages)
    {
        std::cout << "[" << idx << "] element, yaw = " << r.angle_y << ", pitch = " << r.angle_p << ", roll = " << r.angle_r << std::endl;
    }

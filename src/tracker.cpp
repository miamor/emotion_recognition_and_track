...
TrackerParams::TrackerParams()
    : //   min_track_duration(1000),
      ...
      min_det_conf(0.3f),
      bbox_aspect_ratios_range(0.0f, 20.0f),
      bbox_heights_range(0.1, 1080),
      predict(55),
...
{
    PT_CHECK_GE(p.min_track_duration, static_cast<size_t>(50));
    ...
    PT_CHECK_LE(p.bbox_aspect_ratios_range[1], 20.0f);
    PT_CHECK_LT(p.bbox_aspect_ratios_range[0], p.bbox_aspect_ratios_range[1]);

    PT_CHECK_GE(p.bbox_heights_range[0], 0.1f);
...
//   |tp fn|
//   |fp tn|
cv::Mat Tracker::ConfusionMatrix(const std::vector<Match> &matches)
{
...
const TrackerParams &Tracker::params() const { return params_; }
...
const Tracker::Descriptor &Tracker::descriptor_fast() const
{
    return descriptor_fast_;
}

// Descriptor fast setter.
void Tracker::set_descriptor_fast(const Descriptor &val)
{
    descriptor_fast_ = val;
}

// Descriptor strong getter.
const Tracker::Descriptor &Tracker::descriptor_strong() const
{
    return descriptor_strong_;
}

// Descriptor strong setter.
void Tracker::set_descriptor_strong(const Descriptor &val)
{
    descriptor_strong_ = val;
}

// Distance fast getter.
const Tracker::Distance &Tracker::distance_fast() const { return distance_fast_; }

// Distance fast setter.
void Tracker::set_distance_fast(const Distance &val) { distance_fast_ = val; }

// Distance strong getter.
const Tracker::Distance &Tracker::distance_strong() const { return distance_strong_; }

// Distance strong setter.
void Tracker::set_distance_strong(const Distance &val) { distance_strong_ = val; }

// Returns all tracks including forgotten (lost too many frames ago).
const std::unordered_map<size_t, Track> &
Tracker::tracks() const
{
    return tracks_;
}

// Returns indexes of active tracks only.
const std::set<size_t> &Tracker::active_track_ids() const
{
    return active_track_ids_;
}

// Returns detection log which is used for tracks saving.
DetectionLog Tracker::GetDetectionLog(const bool valid_only) const
{
...
const std::vector<Tracker::Match> &
Tracker::base_classifier_matches() const
{
...
// and
// shape, motion and time affinity.
const std::vector<Tracker::Match> &Tracker::reid_based_classifier_matches() const
{
    return reid_based_classifier_matches_;
}

// Returns decisions made by strong distance/descriptor affinity.
const std::vector<Tracker::Match> &Tracker::reid_classifier_matches() const
{
    return reid_classifier_matches_;
}

TrackedObjects Tracker::FilterDetections(
...
void Tracker::SolveAssignmentProblem(
    ...
    // std::set<std::tuple<size_t, size_t, float, int, double>> *matches)
    std::set<std::tuple<size_t, size_t, float>> *matches)
{
    ...
    for (auto id : track_ids)
    {
        ...
        {
            // matches->emplace(id, res[i], 1 - dissimilarity.at<float>(i, res[i]), detections[res[i]].label, detections[res[i]].dist);
...
const ObjectTracks Tracker::all_tracks(bool valid_only) const
{
...
cv::Rect Tracker::PredictRect(size_t id, size_t k,
                              size_t s) const
{
...
bool Tracker::EraseTrackIfItWasLostTooManyFramesAgo(
    size_t track_id)
{
    if (tracks_.find(track_id) == tracks_.end())
        return true;
...
bool Tracker::UpdateLostTrackAndEraseIfItsNeeded(
    size_t track_id)
{
                      ...
                      int frame_idx)
{
    ...
    if (!active_tracks.empty() && !detections.empty())
    {
        ...
        // SolveAssignmentProblem(active_tracks, detections, &unmatched_tracks,
...
float Tracker::ShapeAffinity(float weight, const cv::Rect &trk,
                             const cv::Rect &det)
{
...
float Tracker::MotionAffinity(float weight, const cv::Rect &trk,
                              const cv::Rect &det)
{
...
std::vector<float> Tracker::ComputeDistances(
    const cv::Mat &frame,
...
Tracker::GetTrackToDetectionIds(
    // const std::set<std::tuple<size_t, size_t, float, int, double>> &matches)
...
void Tracker::AddNewTrack(const cv::Mat &frame,
...
void Tracker::AppendToTrack(const cv::Mat &frame,
                            size_t track_id,
                            const TrackedObject &detection,
                            const cv::Mat &descriptor_fast,
                            const cv::Mat &descriptor_strong)
{
    ...
    // detection_with_id.label = label;
    // detection_with_id.dist = dist;
...
float Tracker::AffinityFast(const cv::Mat &descriptor1,
...
float Tracker::Affinity(const TrackedObject &obj1,
                        const TrackedObject &obj2)
{
    float shp_aff = ShapeAffinity(params_.shape_affinity_w, obj1.rect, obj2.rect);
    float mot_aff =
...
bool Tracker::IsTrackForgotten(const Track &track) const
{
...
TrackedObjects Tracker::TrackedDetectionsWithLabels() const
{
    ...
    for (size_t idx : active_track_ids())
    {
        ...
        if (IsTrackValid(idx) && !track.lost)
        {
            ...
            int counter = 1;
            size_t start = static_cast<int>(track.objects.size()) >= params_.averaging_window_size_for_rects ? track.objects.size() - params_.averaging_window_size_for_rects : 0;
...
int LabelWithMaxFrequencyInTrack(const Track &track, int window_size)
{
    ...
    int start = static_cast<int>(track.objects.size()) >= window_size ? static_cast<int>(track.objects.size()) - window_size : 0;
...
cv::Mat Tracker::DrawActiveTracks(const cv::Mat &frame)
{
    ...
    for (auto active_track : active_tracks)
    {
        ...
        std::stringstream ss;
        ss << idx;
        // cv::putText(out_frame, ss.str(), centers.back(), cv::FONT_HERSHEY_SCRIPT_COMPLEX, 1.0, colors_[idx % colors_.size()], 2);
        cv::putText(out_frame, ss.str(), centers.back(), cv::FONT_HERSHEY_PLAIN, 2.0, colors_[idx % colors_.size()], 2);
        auto track = tracks().at(idx);
        if (track.lost)
        {
            cv::line(out_frame, active_track.second.back(),
                     Center(track.predicted_rect), cv::Scalar(0, 0, 0), 2);
        }
    }
...
void Tracker::PrintConfusionMatrices() const
{
...
void Tracker::PrintReidPerformanceCounts() const
{
